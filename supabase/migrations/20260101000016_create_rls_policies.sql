...
-- Apply to all tables with updated_at (drop first so re-runs / partial applies succeed)
DROP TRIGGER IF EXISTS set_updated_at ON construction.companies;
DROP TRIGGER IF EXISTS set_updated_at ON construction.profiles;
DROP TRIGGER IF EXISTS set_updated_at ON construction.projects;
DROP TRIGGER IF EXISTS set_updated_at ON construction.materials;
DROP TRIGGER IF EXISTS set_updated_at ON construction.employees;
DROP TRIGGER IF EXISTS set_updated_at ON construction.suppliers;
DROP TRIGGER IF EXISTS set_updated_at ON construction.purchase_orders;
DROP TRIGGER IF EXISTS set_updated_at ON construction.payroll;
DROP TRIGGER IF EXISTS set_updated_at ON construction.equipment;
...
-- ============================================================
-- Helper functions (construction schema — cannot CREATE in auth.*)
-- ============================================================

-- Helper: get current user's company_id (via company_members)
CREATE OR REPLACE FUNCTION construction.user_company_id()
RETURNS UUID AS $$
...
CREATE OR REPLACE FUNCTION construction.user_role()
RETURNS TEXT AS $$
...
CREATE OR REPLACE FUNCTION construction.is_company_admin()
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
...
-- PROFILES policies
-- ============================================================
DROP POLICY IF EXISTS "Users view own company profiles" ON construction.profiles;
...
DROP POLICY IF EXISTS "Users update own profile" ON construction.profiles;
CREATE POLICY "Users update own profile"
  ON construction.profiles FOR UPDATE
  USING (id = auth.uid())
  WITH CHECK (id = auth.uid());

DROP POLICY IF EXISTS "Owner/Admin update any profile in company" ON construction.profiles;
CREATE POLICY "Owner/Admin update any profile in company"
  ON construction.profiles FOR UPDATE
  USING (
    construction.is_company_admin()
    AND id IN (
      SELECT user_id FROM construction.company_members
      WHERE company_id = construction.user_company_id()
    )
...
DROP POLICY IF EXISTS "Owner/Admin update company" ON construction.companies;
CREATE POLICY "Owner/Admin update company"
  ON construction.companies FOR UPDATE
  USING (id = construction.user_company_id() AND construction.is_company_admin());
...
-- ============================================================
DROP POLICY IF EXISTS "View own company members" ON construction.company_members;
CREATE POLICY "View own company members"
  ON construction.company_members FOR SELECT
  USING (company_id = construction.user_company_id());

DROP POLICY IF EXISTS "Owner/Admin manage members" ON construction.company_members;
CREATE POLICY "Owner/Admin manage members"
  ON construction.company_members FOR ALL
  USING (
    company_id = construction.user_company_id()
    AND construction.is_company_admin()
  );
...
-- ============================================================
DROP POLICY IF EXISTS "Owner/Admin manage invitations" ON construction.company_invitations;
...
DROP POLICY IF EXISTS "Invited user can view own invitation" ON construction.company_invitations;
...
CREATE POLICY "Company members view projects"
  ...
  USING (
    company_id = construction.user_company_id()
    AND (
      construction.user_role() IN ('owner', 'admin', 'accountant', 'procurement_officer')
      OR id IN (
...
DROP POLICY IF EXISTS "Company isolation for projects" ON construction.projects;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for project_members" ON construction.project_members;
CREATE POLICY "Company isolation for project_members"
  ON construction.project_members FOR ALL
  USING (
    project_id IN (
      SELECT id FROM construction.projects WHERE company_id = construction.user_company_id()
    )
  );
...
-- MATERIALS policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for materials" ON construction.materials;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for project_inventory" ON construction.project_inventory;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for stock_reservations" ON construction.stock_reservations;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for inventory_transfers" ON construction.inventory_transfers;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for inventory_logs" ON construction.inventory_logs;
...
-- SUPPLIERS policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for suppliers" ON construction.suppliers;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for purchase_orders" ON construction.purchase_orders;
CREATE POLICY "Company isolation for purchase_orders"
  ON construction.purchase_orders FOR SELECT
  USING (company_id = construction.user_company_id());

DROP POLICY IF EXISTS "Create POs" ON construction.purchase_orders;
CREATE POLICY "Create POs"
  ON construction.purchase_orders FOR INSERT
  WITH CHECK (
    company_id = construction.user_company_id()
    AND construction.user_role() IN ('owner', 'admin', 'procurement_officer')
  );

DROP POLICY IF EXISTS "Update POs" ON construction.purchase_orders;
CREATE POLICY "Update POs"
  ON construction.purchase_orders FOR UPDATE
  USING (company_id = construction.user_company_id());

DROP POLICY IF EXISTS "Delete POs" ON construction.purchase_orders;
CREATE POLICY "Delete POs"
  ON construction.purchase_orders FOR DELETE
  USING (company_id = construction.user_company_id() AND construction.is_company_admin());

-- ============================================================
-- PO_ITEMS policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for po_items" ON construction.po_items;
CREATE POLICY "Company isolation for po_items"
  ON construction.po_items FOR ALL
  USING (
    purchase_order_id IN (
      SELECT id FROM construction.purchase_orders WHERE company_id = construction.user_company_id()
    )
  );
...
-- DELIVERIES policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for deliveries" ON construction.deliveries;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for delivery_items" ON construction.delivery_items;
CREATE POLICY "Company isolation for delivery_items"
  ...
  USING (
    delivery_id IN (
      ...
      WHERE po.company_id = construction.user_company_id()
    )
  );
...
-- EMPLOYEES policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for employees" ON construction.employees;
...
-- ATTENDANCE policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for attendance" ON construction.attendance;
CREATE POLICY "Company isolation for attendance"
  ON construction.attendance FOR ALL
  USING (
    employee_id IN (
      SELECT id FROM construction.employees WHERE company_id = construction.user_company_id()
    )
  );
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for cash_advances" ON construction.cash_advances;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for employee_loans" ON construction.employee_loans;
...
-- PAYROLL policies
-- ============================================================
DROP POLICY IF EXISTS "Admin/Accountant view all payroll" ON construction.payroll;
CREATE POLICY "Admin/Accountant view all payroll"
  ON construction.payroll FOR SELECT
  USING (
    company_id = construction.user_company_id()
    AND construction.user_role() IN ('owner', 'admin', 'accountant')
  );

DROP POLICY IF EXISTS "Admin/Accountant manage payroll" ON construction.payroll;
...
DROP POLICY IF EXISTS "Admin/Accountant manage payroll items" ON construction.payroll_items;
CREATE POLICY "Admin/Accountant manage payroll items"
  ON construction.payroll_items FOR ALL
  USING (
    payroll_id IN (
      SELECT id FROM construction.payroll WHERE company_id = construction.user_company_id()
    )
    AND construction.user_role() IN ('owner', 'admin', 'accountant')
  );
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for project_costs" ON construction.project_costs;
...
-- EXPENSES policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for expenses" ON construction.expenses;
...
-- EQUIPMENT policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for equipment" ON construction.equipment;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for equipment_assignments" ON construction.equipment_assignments;
CREATE POLICY "Company isolation for equipment_assignments"
  ON construction.equipment_assignments FOR ALL
  USING (
    equipment_id IN (
      SELECT id FROM construction.equipment WHERE company_id = construction.user_company_id()
    )
  );
...
-- DOCUMENTS policies
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for documents" ON construction.documents;
...
-- ============================================================
DROP POLICY IF EXISTS "Users view own notifications" ON construction.notifications;
CREATE POLICY "Users view own notifications"
  ON construction.notifications FOR SELECT
  USING (user_id = auth.uid() AND company_id = construction.user_company_id());

DROP POLICY IF EXISTS "System can insert notifications" ON construction.notifications;
CREATE POLICY "System can insert notifications"
  ON construction.notifications FOR INSERT
  WITH CHECK (company_id = construction.user_company_id());

DROP POLICY IF EXISTS "Users update own notifications" ON construction.notifications;
...
-- AUDIT_LOGS policies
-- ============================================================
DROP POLICY IF EXISTS "Admin only audit logs" ON construction.audit_logs;
CREATE POLICY "Admin only audit logs"
  ON construction.audit_logs FOR SELECT
  USING (
    company_id = construction.user_company_id()
    AND construction.user_role() IN ('owner', 'admin')
  );

DROP POLICY IF EXISTS "System can insert audit logs" ON construction.audit_logs;
...
-- ============================================================
DROP POLICY IF EXISTS "Company isolation for subscriptions" ON construction.subscriptions;
